...
# Logging stuff.
e_header()   { echo -e "\n\033[1m$@\033[m"; }
e_success()  { echo -e " \033[1;32mOK\033[m  $@"; }
e_error()    { echo -e " \033[1;31mNO\033[m  $@"; }
...
prompt_menu() {
  ...
  if [[ "$exitcode" == 0 ]]; then
    ...
    while clear; _prompt_menu_draws "$1" 1 && read -rp "$prompt" nums && [[ "$nums" ]]; do
